...
Panther::Encryptor::Encryptor(const std::string &secret_key_file_path, const std::string &public_key_file_path, long plaintextModulus, long phiM, long lifting, long numOfBitsOfModulusChain, long numOfColOfKeySwitchingMatrix)
    ...
    Panther::FileSystem sk_fs(secret_key_file_path);
    sk_fs.open_output_stream(std::fstream::trunc);
    std::ofstream &sk_fs_of = sk_fs.get_output_stream();

    Panther::FileSystem pk_fs(public_key_file_path);
...
Panther::Encryptor::Encryptor(const std::string &private_key_file_path, const std::string &public_key_file_path, long plaintextModulus, long lifting, long numOfBitsOfModulusChain, long numOfColOfKeySwitchingMatrix, long desiredSlotCount, long securityLevel)
...
Panther::Encryptor::Encryptor(const std::string &secret_key_file_path, const std::string &public_key_file_path) {
    Panther::FileSystem sk_fs(secret_key_file_path);
...
void
Panther::Encryptor::testEncryption() {
...
void
Panther::Encryptor::decryptAndPrint(const std::string &msg, const helib::Ctxt &ctxt) {
...
void
Panther::Encryptor::decryptAndPrintCondensed(const std::string &msg, const helib::Ctxt &ctxt, int size){
...
void
Panther::Encryptor::decryptWrapper(const helib::Ctxt &ctxt, std::vector<long> * dec_ptxt){
    // std::vector<long> dec_ptxt(this->getEncryptedArray()->size());
    this->getEncryptedArray()->decrypt(ctxt, *(this->secret_key), *(dec_ptxt));
}

helib::Context *
Panther::Encryptor::getContext() const {
    return context;
}

helib::SecKey *
Panther::Encryptor::getSecretKey() const {
    return secret_key;
}

helib::PubKey *
Panther::Encryptor::getPublicKey() const {
    return public_key;
}

helib::EncryptedArray *
Panther::Encryptor::getEncryptedArray() const {
    return encrypted_array;
}

int
Panther::Encryptor::getSlotCount() {
